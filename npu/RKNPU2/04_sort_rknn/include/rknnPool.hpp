...
int rknn_lite::interf()
{
    ...
    static TrackingSession *sess = CreateSession(2, 3, 0.3);
    std::map<int, std::vector<DetectionBox>> detection_boxes;
    for (int i = 0; i <  io_num.n_output; ++i)
    {
    ...
    for (int i = 0; i < detect_result_group.count; i++)
    {
        ...
        det.box = cv::Rect_<float>(cv::Point_<float>(x1, y1), cv::Point_<float>(det_result->box.right, det_result->box.bottom));
        det.score = det_result->prop;
        detection_boxes[frame_id].push_back(det);
    }
    auto dets = detection_boxes[frame_id];                                                                                                                      
    frame_id++;                                                                                                                                                    
    auto trks = sess->Update(dets);                                                                                                                                
    for(auto &trk : trks)                                                                                                                                          
    {                                                                                                                                                              
        cv::rectangle(ori_img, trk.box,  cv::Scalar(0, 0, 255, 0), 2, 8, 0);                                                                                       
        putText(ori_img, std::to_string(trk.id), cv::Point(trk.box.x, trk.box.y+ 12), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255));                   
    }                                                                                                                                                              
